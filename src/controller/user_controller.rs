...
pub async fn login(login_body:JsonBody<LoginReq>)->Res<LoginRes>{
  if let Some(code) = login_body.code.clone(){
    match get_captcha_by_code(code).await{
      Ok(captcha_one_option)=>{
        if let Some(_) = captcha_one_option{
          if let (Some(username),Some(password)) = (login_body.username.clone(),login_body.password.clone()){
            match get_user_by_up(username,password).await{
              Ok(r_op)=>{
